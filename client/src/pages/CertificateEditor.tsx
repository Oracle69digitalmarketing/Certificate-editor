...
const useCertificateStore = create<EditorState>((set) => ({
  step: "upload",
  debug: false,
  templateImage: null,
...
export default function CertificateEditor() {
  const { step, reset, originalPdfBytes, pdfWidth, pdfHeight, fields, templateImage, debug, toggleDebug } = useCertificateStore();
  ...
  const exportTruePDF = async () => {
    ...
    try {
      ...
      fields.forEach((field) => {
         let selectedFont = fontRegular;
         if (field.isBold && field.isItalic) selectedFont = fontBoldItalic;
         else if (field.isBold) selectedFont = fontBold;
         else if (field.isItalic) selectedFont = fontItalic;
      ...
      <div className="h-24 bg-white border-b border-slate-200 flex items-center justify-between px-10 z-50 shadow-sm shrink-0">
         <div className="flex items-center gap-12">
            <div className="flex items-center gap-4">
               ...
               <span className="font-black text-slate-900 text-2xl tracking-tighter">Document Engine</span>
            </div>
            <div className="hidden lg:flex items-center gap-10">
               {[
                 ...
                 { id: "preview", label: "Render Preview", icon: Eye }
                  ...
                  <div key={s.id} className={`flex items-center gap-3 font-black text-xs uppercase tracking-[0.2em] transition-all cursor-pointer ${step === s.id ? "text-indigo-600 border-b-4 border-indigo-600 pb-1" : "text-slate-300 hover:text-slate-400"}`} onClick={() => useCertificateStore.getState().setStep(s.id as EditorStep)}>
         ...
         <div className="flex items-center gap-6">
            {step === "scan" ? (
               <Button onClick={() => useCertificateStore.getState().setStep("edit")} className="bg-slate-900 text-white font-black rounded-2xl px-10 h-14 hover:bg-slate-800 text-lg shadow-xl transition-all hover:scale-105 active:scale-95">Go to Editor <ChevronRight className="w-6 h-6 ml-2" /></Button>
            ) : step === "edit" ? (
               <Button onClick={() => useCertificateStore.getState().setStep("preview")} className="bg-slate-900 text-white font-black rounded-2xl px-10 h-14 hover:bg-slate-800 text-lg shadow-xl transition-all hover:scale-105 active:scale-95">Verify & Preview <ChevronRight className="w-6 h-6 ml-2" /></Button>
            ) : (
               <>
                 <Button variant="ghost" onClick={() => useCertificateStore.getState().setStep("edit")} className="font-black text-slate-500 rounded-2xl px-8 h-14 text-lg"><ChevronLeft className="w-6 h-6 mr-2" /> Back to Matrix</Button>
...
function UploadView() {
  ...
  const { loadTemplate, addScanLog } = useCertificateStore();
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const detectType = (str: string, fontSize: number) => {
         ...
         // Capitalized words without digits are likely names or titles
         if (s.length > 3 && s === s.toUpperCase() && !/\d/.test(s)) return "Recipient Name";
         
         // Title case (First word capitalized)
         if (/^[A-Z][a-z]+(\s[A-Z][a-z]+)+$/.test(s)) {
            // If font size is larger than median, it's a strong candidate for Name
            return fontSize > medianFontSize * 1.1 ? "Name Candidate" : "Text Node";
         }
        ...
        setTimeout(() => {
          ...
          if (topElements.length === 0) {
             const fallback = [...scannedElements]
               ...
               .slice(0, 4);
             fallback.forEach(f => useCertificateStore.getState().activateField(f.id));
          } else {
             topElements.forEach(f => useCertificateStore.getState().activateField(f.id));
          }
        }, 100);
...
function FormSidebar() {
  const { step, fields, scannedElements, selectedId, setSelectedId, updateField, addField, removeField, pdfWidth, pdfHeight, debug } = useCertificateStore();
        ...
        <div className="space-y-6 pb-20">
          {fields.length === 0 && (
             <div className="py-20 text-center space-y-6 px-6 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                {step !== "scan" && (
                  <Button variant="outline" onClick={() => useCertificateStore.getState().setStep("scan")} className="rounded-xl border-2 border-indigo-100 text-indigo-600 font-black px-8 mt-4 hover:bg-indigo-50 shadow-sm">
          ...
          {fields.map((field) => (
            <motion.div key={field.id} className={`p-8 rounded-[3rem] border-2 transition-all cursor-pointer relative group ${selectedId === field.id ? "border-indigo-600 bg-indigo-50 shadow-xl ring-[12px] ring-indigo-50" : "border-slate-50 bg-[#fafafa] hover:border-slate-200"}`} onClick={() => setSelectedId(field.id)} onMouseEnter={() => setSelectedId(field.id)}>
              <div className="flex items-center justify-between mb-5">
                <div className="space-y-1">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{field.type} | Ref</span>
                  <p className="text-xs font-bold text-slate-500 truncate max-w-[300px] italic">"{field.originalValue || "N/A"}"</p>
                </div>
                <Button size="icon" variant="ghost" className="h-12 w-12 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all" onClick={(e) => { e.stopPropagation(); removeField(field.id); }}>
                  <Trash2 className="w-6 h-6" />
                </Button>
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Replacement Value</Label>
                <Input value={field.value} onChange={(e) => updateField(field.id, { value: e.target.value })} className="bg-white border-slate-200 h-16 rounded-[1.5rem] font-black text-xl text-slate-900 shadow-sm focus:ring-4 focus:ring-indigo-100" />
              </div>
              
              {selectedId === field.id && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 pt-8 border-t border-indigo-100 grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Font Size (pt)</Label>
                    <Input type="number" value={Math.round(field.fontSize)} onChange={(e) => updateField(field.id, { fontSize: parseInt(e.target.value) })} className="h-12 rounded-2xl font-black text-lg" />
                  </div>
                  <div className="flex items-end justify-between">
                    <Button variant="ghost" size="icon" className={`h-12 w-12 rounded-2xl ${field.isBold ? "bg-indigo-600 text-white shadow-xl" : "bg-white border-2 border-slate-100"}`} onClick={() => updateField(field.id, { isBold: !field.isBold })}>B</Button>
                    <Button variant="ghost" size="icon" className={`h-12 w-12 rounded-2xl italic ${field.isItalic ? "bg-indigo-600 text-white shadow-xl" : "bg-white border-2 border-slate-100"}`} onClick={() => updateField(field.id, { isItalic: !field.isItalic })}>I</Button>
                    <Button variant="ghost" size="icon" className={`h-12 w-12 rounded-2xl ${field.useMask ? "bg-orange-500 text-white shadow-xl" : "bg-white border-2 border-slate-100"}`} onClick={() => updateField(field.id, { useMask: !field.useMask })} title="Toggle Smart Mask">M</Button>
                  </div>
                </motion.div>
              )}
            </motion.div>
...
function CanvasRenderer() {
  const { step, templateImage, pdfWidth, pdfHeight, fields, scannedElements, selectedId, setSelectedId, updatePosition, activateField, addField, debug } = useCertificateStore();
